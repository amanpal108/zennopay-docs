...
curl -X POST https://api.zennopay.com/v1/payment_intents \
  ...
  -d '{"amount_usd_cents":345,"corridor":"th_gln"}'
```
...
const body = JSON.stringify({ amount_usd_cents: 345, corridor: "th_gln" });
const bodyHash = crypto.createHash("sha256").update(body).digest("hex");

const canonical = [method, path, timestamp, nonce, bodyHash].join("\n");
const signature = crypto.createHmac("sha256", secret).update(canonical).digest("base64");

const res = await fetch("https://api.zennopay.com" + path, {
  method,
  headers: {
...
| 400 | `invalid_corridor` | Corridor must be `th_gln` or `vn_9pay` |
